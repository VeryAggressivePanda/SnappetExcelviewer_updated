...
function renderNode(node, level, expandStateMap = {}) {
  ...
  // Add column type for list template styling
  if (node.columnType) {
    nodeEl.setAttribute('data-column-type', node.columnType);
  }
  
  // Add bullet marker for CSS targeting
  if (node.isBullet) {
    nodeEl.setAttribute('data-is-bullet', 'true');
  }
  ...
  // Prevent week numbers from splitting in titles too
  if (displayText && typeof displayText === 'string') {
    displayText = displayText.replace(/\b(week)\s+(\d+)\b/gi, '$1\u00A0$2');
  }
  
  title.innerHTML = (displayText + templateIndicator);
  ...
  if (node.children && node.children.length > 0) {
    ...
    let cellValue = node.value || '';
    
    // Prevent week numbers from splitting (add non-breaking space between "week" and number)
    if (cellValue && typeof cellValue === 'string') {
      cellValue = cellValue.replace(/\b(week)\s+(\d+)\b/gi, '$1\u00A0$2');
    }
    
    content.innerHTML = cellValue; // Use innerHTML to support &nbsp;
    
    if (isReallyEmpty) {
      content.classList.add(`level-${cssLevel}-empty-content`);
    }
    
    // Add bullet marker to content for CSS targeting
    if (node.isBullet) {
      content.setAttribute('data-is-bullet', 'true');
    }
  }
