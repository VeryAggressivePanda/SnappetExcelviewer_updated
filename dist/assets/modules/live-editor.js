...
function addEditControls(node, nodeEl, header) {
  ...
  addContentBtn.addEventListener('click', (e) => {
    ...
    // Use old logic: only allow template nodes or truly empty nodes (not duplicates)
    // Unless it's a list template column which has different rules
    if (!node.isListColumn) {
  ...
  if (hasContent) {
    ...
    layoutToggleBtn.addEventListener('click', (e) => {
      ...
      if (node.isDuplicate && !node.isListColumn) {
  ...
  // 2.5. Parse/Filter Button (only for list columns with data)
  if (node.isListColumn && hasChildren) {
    const parseFilterBtn = document.createElement('button');
    parseFilterBtn.className = 'icon-button parse-filter-button';
    parseFilterBtn.innerHTML = '⚙️';
    parseFilterBtn.title = 'Data Parse Opties';
    
    parseFilterBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      showParseOptionsPopup(node, e.target);
    });
    
    editControls.appendChild(parseFilterBtn);
  }
...
function addChildContainer(parentNode, skipReRender = false) {
  // Use old logic: only allow template nodes or truly empty nodes (not duplicates)
  // Unless it's a list template column which has different rules
  if (!parentNode.isListColumn) {
    const isTemplateOrEmpty = parentNode.isTemplate || 
                             (!parentNode.columnIndex && parentNode.columnIndex !== 0) || 
                             parentNode.isPlaceholder ||
                             (parentNode.value && (parentNode.value === 'Container' || parentNode.value === 'New Container' || parentNode.value === 'Start Building'));
...
function assignColumnToNode(node, columnIndex, skipReplication = false) {
  ...
  if (contextualValues.length === 0) {
    ...
    // FORCE this manually created node to be template regardless of what already exists
    node.isTemplate = true;
    node.isDuplicate = false;
    console.log(`🏗️ FORCE TEMPLATE: Manually created "${node.value}" (${node.columnName}) is now TEMPLATE`);
    
    // CRITICAL: Mark as template if this is the first node of this type!
    if (window.markGlobalTemplatesOfType) {
      const activeSheetId = window.excelData.activeSheetId;
      const sheetData = window.excelData.sheetsLoaded[activeSheetId];
      const rootNode = sheetData.root;
      window.markGlobalTemplatesOfType(rootNode, node.columnName, node.columnIndex, node.id);
    }
  } else {
...
function markGlobalTemplatesOfType(rootNode, columnName, columnIndex, newlyCreatedNodeId = null) {
  ...
  // If a newly created node is specified, it should become the template
  let template;
  if (newlyCreatedNodeId) {
    template = allNodesOfType.find(node => node.id === newlyCreatedNodeId);
    if (!template) {
      console.warn('🔍 Newly created node not found, using first node as template');
      template = allNodesOfType[0];
    }
  } else {
    // Fallback: First node becomes the template
    template = allNodesOfType[0];
  }
  
  template.isTemplate = true;
  template.isDuplicate = false;
  console.log(`🏗️ master GLOBAL: Marked "${template.value}" (${columnName}) as GLOBAL template`);
  
  // All others become duplicates
  allNodesOfType.forEach(node => {
    if (node.id !== template.id) {
      node.isDuplicate = true;
      node.isTemplate = false;
      node.templateId = template.id;
      console.log(`📋 GLOBAL: Marked "${node.value}" (${columnName}) as GLOBAL duplicate`);
    }
  });
}
...
// List Template: Populate column with parsed data
function populateListTemplateColumn(node, selectedColumnIndices, parseMode, sheetData) {
  console.log('📝 Populating list template column:', { parseMode, selectedColumnIndices });
  ...
  // Sort column indices to ensure logical order (Blok, Week, Les, etc.)
  const sortedColumnIndices = [...selectedColumnIndices].sort((a, b) => a - b);
  
  // Process each row
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
    
    // Combine selected columns in sorted order
    const combinedValue = sortedColumnIndices
      .map(colIndex => row[colIndex] || '')
      .filter(val => val.trim() !== '')
      .join(', ');
    
    if (combinedValue) {
      const parsedValue = parseValueSimple(combinedValue, parseMode);
  ...
  const uniqueItems = [...new Set(parsedItems)].sort();
  
  // Create child nodes
  uniqueItems.forEach((item, index) => {
    const childNode = {
      id: `list-item-${Date.now()}-${index}`,
      value: item,
      columnName: `Item ${index + 1}`,
      level: 1,
      children: [],
      properties: []
    };
  ...
  // Store original column indices for re-parsing (use sorted order)
  node.originalColumnIndices = sortedColumnIndices;
  node.parseMode = parseMode; // Store current parse mode
  
  // Update column name display (use sorted order)
  const columnNames = sortedColumnIndices.map(index => headers[index]).join(', ');
  node.value = `${columnNames} (${uniqueItems.length} items)`;
  
  // FORCE EXPAND the node so data is visible
  ...
  console.log(`✅ Populated ${node.value} with ${uniqueItems.length} items`);
}

// Simple parse function
function parseValueSimple(value, mode) {
  if (!value || typeof value !== 'string') return '';
  
  switch (mode) {
    case 'before_colon':
      const colonIndex = value.indexOf(':');
      return colonIndex > -1 ? value.substring(0, colonIndex).trim() : value;
      
    case 'after_colon':
      const colonIndex2 = value.indexOf(':');
      return colonIndex2 > -1 ? value.substring(colonIndex2 + 1).trim() : '';
...
// Show parse options popup near the filter icon
function showParseOptionsPopup(node, iconElement) {
  // Remove existing popup if any
  const existingPopup = document.querySelector('.parse-options-popup');
  if (existingPopup) {
  ...
  popup.className = 'parse-options-popup';
  popup.innerHTML = `
    <div class="popup-header">📊 Data Parse Opties</div>
    <label class="parse-option">
      <input type="radio" name="parseMode" value="full" ${(!node.parseMode || node.parseMode === 'full') ? 'checked' : ''}>
      🔸 Show Everything
    </label>
    <label class="parse-option">
      <input type="radio" name="parseMode" value="before_colon" ${node.parseMode === 'before_colon' ? 'checked' : ''}>
      🔸 Before : (bijv. "per tweetal")
    </label>
    <label class="parse-option">
      <input type="radio" name="parseMode" value="after_colon" ${node.parseMode === 'after_colon' ? 'checked' : ''}>
      🔸 After : (bijv. "10 potloden, schoen")
    </label>
  `;
  ...
  popup.style.left = (iconRect.right - 200) + 'px'; // Align right edge
  
  // Add event listeners for radio buttons
  popup.addEventListener('change', (e) => {
    if (e.target.type === 'radio') {
      const newParseMode = e.target.value;
      console.log(`🔄 Changing parse mode for ${node.value} to: ${newParseMode}`);
      
      // Store the parse mode on the node
      node.parseMode = newParseMode;
      
      // Re-populate the column with new parse mode
      const activeSheetId = window.excelData.activeSheetId;
      const sheetData = window.excelData.sheetsLoaded[activeSheetId];
      
      // Get the original column indices (we need to store these)
      if (node.originalColumnIndices) {
        populateListTemplateColumn(node, node.originalColumnIndices, newParseMode, sheetData);
      }
...
function showColumnSelectionArea(node) {
    ...
    // Hide any existing column selection area
    hideColumnSelectionArea();
    
    console.log('showColumnSelectionArea called with node:', node);
    
    // Check if we're in automatic list template mode
    const currentSheetData = getCurrentSheetData();
    const isAutomatic = currentSheetData && currentSheetData.isListTemplate && currentSheetData.processedData;
    
    if (isAutomatic) {
        console.log('🤖 Automatic list template mode detected - no manual column selection needed');
        showInfoMessage('Deze kolom wordt automatisch gevuld op basis van Excel data analyse.');
        return;
    }
    
    // Manual column selection logic works for:
    // 1. Hierarchy mode (not list template)
    // 2. List template mode with list columns
    const debugInfo = {
        hasSheetData: !!currentSheetData,
        isListTemplate: currentSheetData?.isListTemplate,
        isListColumn: node?.isListColumn,
        isHierarchyMode: !currentSheetData?.isListTemplate
    };
    
    console.log('🔍 Debug Column Selection:', debugInfo);
    
    // Allow column selection in hierarchy mode OR in list template with list columns
    const shouldShowSelection = !currentSheetData?.isListTemplate || (currentSheetData?.isListTemplate && node?.isListColumn);
    
    if (!shouldShowSelection) {
        console.log('❌ Column selection not allowed for this node type');
        return;
    }
    ...
    console.log('🎯 Column selection area created:', columnSelectionArea);
    console.log('🎯 Parse options included:', currentSheetData?.isListTemplate);
...
function setupAddButtonForArea(area) {
  ...
  addButton.addEventListener('click', () => {
    console.log('🚨 ADD BUTTON CLICKED! Selected columns:', selectedColumnIndices);
    console.trace('🚨 ADD BUTTON CLICK STACK TRACE');
    ...
    // Check if this is list template mode
    const activeSheetId = window.excelData.activeSheetId;
    const sheetData = window.excelData.sheetsLoaded[activeSheetId];
    const isListTemplate = sheetData && sheetData.isListTemplate && savedSelectedNode.isListColumn;
    
    if (isListTemplate) {
      // List template mode: populate with parsed data (default to 'full')
      const parseMode = 'full';
      console.log('🎯 Using default parse mode:', parseMode);
      populateListTemplateColumn(savedSelectedNode, savedSelectedIndices, parseMode, sheetData);
    } else if (savedSelectedIndices.length === 1) {
      // Single column selection (hierarchy mode)
...
function getCurrentSheetData() {
    ...
    return window.sheetData?.[activeSheetId] || window.excelData?.sheetsLoaded?.[activeSheetId];
}